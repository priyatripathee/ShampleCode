...
{
    public partial class addviewcorporate : System.Web.UI.Page
    {
        Int32 blogId = 0;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (Upload_LargeImages.HasFile)
            {
                ...
                Upload_LargeImages.SaveAs(Server.MapPath("~/images/corp_img/") + fileName + "_" + strbannerUploadTime + ext);
            ...
            if (ViewState["blogId"] != null)
            {
                ...
                {
                    string imagepath = Server.MapPath("~//images/corp_img//" + ViewState["imgName"].ToString());
            ...
            tools_handler toolsHandler = new tools_handler();
            int result = toolsHandler.insert_update_corporate(blogId, txtDescription.Text, LargeNoImage);
            if (result > 0)
            {
                if (ViewState["blogId"] != null)
                {
        ...
        protected void gvLookbook_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            tools_handler toolsHandler = new tools_handler();
            bool delete = toolsHandler.delete_corporate(lid);
            if (delete)
            {
                this.BindLookbook();
        ...
        protected void gvLookbook_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "EditRecored")
            {
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        imgLarge.ImageUrl = "~/images/corp_img/" + dt.Rows[0]["image"].ToString();
            ...
            {
                bool status = BLL.Utility.change_update_status(Convert.ToInt32(e.CommandArgument), true, "is_active", "corporateId", "tbl_corporate");
            }
            if (e.CommandName == "Deactive")
            {
                bool status = DAL.Utility.change_update_status(Convert.ToInt32(e.CommandArgument), false, "is_active", "corporateId", "tbl_corporate");
            }
            this.BindLookbook();
        }
